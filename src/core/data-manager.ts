...
class DataManager {
  ...
  getSearchEngineById(id: string): SearchEngine | undefined {
    return this.getSearchEngines().find(engine => engine.id === id);
  }
  ...
  getDefaultEngine(): SearchEngine {
    ...
    const defaultEngine = this.getSearchEngineById(data.defaultEngine);
    if (!defaultEngine) {
      // 如果找不到默认引擎，返回第一个
  ...
  setDefaultEngine(engineId: string): void {
    const engine = this.getSearchEngineById(engineId);
    if (!engine) {
      throw new Error(`搜索引擎不存在: ${engineId}`);
    }

    this.data!.defaultEngine = engineId;
    this.saveData();
    eventBus.publish('defaultEngine:changed', engineId);
  }
  ...
  addSearchEngine(engine: SearchEngine): void {
    // 确保ID不重复
    if (this.getSearchEngineById(engine.id)) {
      throw new Error(`搜索引擎ID已存在: ${engine.id}`);
    }
   ...
   */
  updateSearchEngine(engineId: string, updatedEngine: Partial<SearchEngine>): void {
    const index = this.data!.engines.findIndex(engine => engine.id === engineId);
    if (index === -1) {
      throw new Error(`搜索引擎不存在: ${engineId}`);
    }

    // 更新引擎，保留ID
    this.data!.engines[index] = {
      ...this.data!.engines[index],
      ...updatedEngine,
      id: engineId // 确保ID不变
    };
  ...
  deleteSearchEngine(engineId: string): void {
    // 不能删除默认引擎
    if (engineId === this.data!.defaultEngine) {
      throw new Error('不能删除默认搜索引擎');
    }
   ...
   * @returns 语言对象
   */
  getLanguages(): Record<string, Record<string, string>> {
  ...
  getDefaultLanguage(): string {
    return this.getData().defaultLanguage;
  }
  ...
  setDefaultLanguage(languageCode: string): void {
    if (!this.data!.languages[languageCode]) {
      throw new Error(`语言不存在: ${languageCode}`);
    }

    this.data!.defaultLanguage = languageCode;
    this.saveData();
    eventBus.publish('defaultLanguage:changed', languageCode);
  }
  ...
  importData(jsonData: string): void {
    try {
      ...
      eventBus.publish('searchEngines:updated', this.getSearchEngines());
      eventBus.publish('shortcuts:updated', this.getShortcuts());
      eventBus.publish('defaultEngine:changed', this.data.defaultEngine);
      eventBus.publish('defaultLanguage:changed', this.data.defaultLanguage);
    } catch (error) {
